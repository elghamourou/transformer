...
<!--10/27/5 This version has been modified to strip out any field repetitions from the SpecXML profile, which are principally identified by the '_rep' field name suffix. This is the new default stylesheet. The original that allows repetition fields is available as an alternative. -->
...
-->
	...
	<xsl:template match="Field">
	    <xsl:variable name="nam" select="substring-before(@Name,'_rep')"/>
	    <xsl:choose>
			<xsl:when test="$nam!=''"></xsl:when>
		<xsl:otherwise>
